  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-display font-black text-navy-950 text-2xl">New Result</h1>
      </div>
        ...
        <form id="main-form" action={formAction} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-xs text-slate-400 mb-3">Upload the old electric bill photo</p>
              <SingleImageUploader name="before_image" />
            </div>
            <div>
              <label className={labelCls}>After</label>
              <p className="text-xs text-slate-400 mb-3">Upload the new electric bill photo</p>
      ...
      <div className="fixed bottom-0 left-64 right-0 bg-white border-t border-slate-200 shadow-[0_-4px_16px_0_rgb(0_0_0/0.06)] px-8 py-4 flex items-center justify-end gap-3 z-30">
        ...
        >
          {isPending ? 'Saving…' : 'Create Result'}
        </button>
      </div>
    </div>
  );
